...
def main(arg):
    ...
    KM_CLUSTERS = int(custom_data['KM_CLUSTERS'])
    ...
    ## DATA
    generator_train = get_generator(json_paths=JSON_PATHS_TRAIN,size=INPUT_DIM,batch_size=IMS_PER_BATCH,damaged=False,km_clusters=KM_CLUSTERS)
    generator_test = get_generator(json_paths=JSON_PATHS_TEST,size=INPUT_DIM,batch_size=IMS_PER_BATCH,damaged=False,km_clusters=KM_CLUSTERS)
    ...
    encoder = Unet(K=1,type='decoder',input_size=INPUT_DIM,do_dropout=USE_DROPOUT,l1_reg=L1_REG,l2_reg=L2_REG)
